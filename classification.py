...
Classifiers = LogisticRegression(max_iter=1000, random_state=0).fit(X_train, y_train)
Prediction = Classifiers.predict(X_test)
LogAccuracy = accuracy_score(y_test, Prediction)
print(f"Logistic Accuracy: {LogAccuracy:.6f}")
model = DecisionTreeClassifier(random_state=0)
model = model.fit(X_train, y_train)
predictions = model.predict(X_test)
DecAccuracy = accuracy_score(y_test, predictions)
print(f"Decision Tree Accuracy:, {DecAccuracy:.6f}")
modelForest = RandomForestClassifier(n_estimators=100, random_state=0)
...
print(f"Random Forest Tree Accuracy:, {ForAccuracy:.6f}")